...
fn load_example(name: &str) -> Workload {
    ...
    serde_yaml::from_str(&content).unwrap_or_else(|e| panic!("parse {}: {e}", path.display()))
}
...
fn labs_kubernetes_workload_parses_and_validates() {
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("examples/labs/kubernetes/workload.yaml");
    ...
    spec.validate()
        .expect("labs kubernetes spec should validate");
}
