...
pub fn classify_task(message: &str) -> TaskClass {
    ...
    if lower.contains("terraform") || lower.contains("ansible") || lower.contains("generate") || lower.contains("code") {
        return TaskClass::CodeGeneration;
    }
    if lower.contains("research") || lower.contains("compare") || lower.contains("finops") || lower.contains("cost") {
        return TaskClass::Research;
    }
    if lower.contains("log") || lower.contains("trace") || lower.contains("metric") || lower.contains("history") {
...
pub fn route_agent(message: &str, agent_override: Option<&str>) -> (String, String, f64, Vec<String>) {
    ...
    if lower.contains("devops") || lower.contains("ci/cd") || lower.contains("pipeline") || lower.contains("gitops") || lower.contains("drift") {
        let p = crate::zeus::agents::get_agent("devops");
        return (p.id, p.label, 0.88, p.suggested_prompts);
    }
    if lower.contains("kubernetes") || lower.contains("k8s") || lower.contains("pod") || lower.contains("cluster") {
    ...
    if lower.contains("observability") || lower.contains("log") || lower.contains("metric") || lower.contains("trace") {
        let p = crate::zeus::agents::get_agent("observability");
        return (p.id, p.label, 0.86, p.suggested_prompts);
    }
    if lower.contains("database") || lower.contains("postgres") || lower.contains("mysql") || lower.contains("redis") {
        let p = crate::zeus::agents::get_agent("database");
        return (p.id, p.label, 0.85, p.suggested_prompts);
    }
    if lower.contains("llm") || lower.contains("gpu") || lower.contains("inference") || lower.contains("model") {
...
pub fn route_message(message: &str, agent_override: Option<&str>) -> RoutingDecision {
    ...
    let (agent_id, agent_label, confidence, suggested_prompts) = route_agent(message, agent_override);
...
pub fn resolve_provider_for_routing(decision: &RoutingDecision, reg: &ZeusProviderRegistry) -> ZeusProviderKind {
