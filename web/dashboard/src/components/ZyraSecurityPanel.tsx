...
export default function SecurityCopilotPanel() {
  ...
  return (
    ...
    >{!report?.suggestions.length ? (
        <p className="text-sm text-subtle">No policy suggestions — threat scan is clean.</p>
      ) : (
            ...
            >
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-lg border glass-divider px-2 py-1 text-xs text-muted hover:border-primary/40"
                >
                  <Copy className="h-3 w-3" />
                  {copied === item.workload ? 'Copied' : 'Copy YAML'}
                </button>
              </div>
              <p className="mb-3 text-sm text-muted">{item.rationale}</p>
