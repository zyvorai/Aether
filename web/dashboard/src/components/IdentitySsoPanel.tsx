...
function ConfigRow({ label, value }: { label: string; value: string | null | undefined }) {
  ...
  return (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:justify-between sm:gap-4 py-2 border-b border-white/5 last:border-0">
      ...
      <code className="text-xs text-slate-300 break-all text-right">{value}</code>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6" data-testid="identity-sso-panel">
      ...
      >
        ...
        <p className="mt-3 text-xs text-slate-500">{settings.session.note}</p>
      </GlassSection>
      ...
      >
        ...
        />
        <p className="mt-3 text-xs text-slate-500">{settings.oidc.note}</p>
      </GlassSection>
      ...
      >
        ...
        <p className="mt-3 text-xs text-slate-500">{settings.saml.note}</p>
      </GlassSection>
      ...
      >
        ...
        />
        <p className="mt-3 text-xs text-slate-500">{settings.ldap.note}</p>
      </GlassSection>
    </div>
  );
}
