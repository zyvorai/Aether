    ...
    <Card className="animate-ac-slide mt-4 overflow-hidden p-0">
      ...
      <div className="flex items-center justify-between px-4 py-3 glass-divider-b glass-inset-surface">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-muted">{workload.runtime}</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onClose} className="text-muted hover:text-foreground text-xl leading-none" aria-label="Close">&times;</button>
        </div>
      </div>

      {showShortcuts ? (
        <div className="px-4 py-2 text-xs text-muted glass-divider-b glass-inset-surface" data-testid="workload-shortcuts-hint">
          <span className="text-muted">Shortcuts:</span>{' '}
          <kbd className="text-muted">1–6</kbd> tabs · <kbd className="text-muted">l</kbd> logs ·{' '}
          <kbd className="text-muted">e</kbd> events · <kbd className="text-muted">o</kbd> overview ·{' '}
          <kbd className="text-muted">s</kbd> exec · <kbd className="text-muted">?</kbd> toggle
        </div>
      ) : null}
      ...
      <div className="p-4">
        {activeTab === 'overview' && (
          <div>
              ...
              <div className="flex gap-2 mb-4 flex-wrap">
                {showShell ? (
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-sm font-medium rounded bg-slate-600/20 text-subtle border border-slate-600/30 cursor-not-allowed"
                  >
                    Exec
                  </button>
                ) : null}
                ...
                {buildResult ? (
                  <p data-testid="workload-build-result" className="w-full text-xs text-muted mt-1">
                    Build: {buildResult}
                  </p>
                ) : null}
              </div>
              ...
              <div className="mb-4 space-y-3">
                ...
                <div className="flex flex-wrap items-end gap-2">
                  ...
                  <button
                    ...
                    disabled={!!actionLoading}
                    className="px-3 py-1.5 text-sm font-medium rounded glass-inset-surface text-muted hover:bg-white/[0.08] border glass-divider disabled:opacity-50"
                  >
                  ...
                  {isScalableClusterWorkload && (
                    <>
                      <div>
                        <label className="mb-1 block text-xs text-subtle">Replicas</label>
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            disabled={!!actionLoading}
                            className="rounded border glass-divider glass-inset-surface px-2 py-1.5 text-sm text-muted hover:bg-white/[0.08] disabled:opacity-50"
                          ...
                          <input
                            ...
                            className="w-16 rounded border glass-divider glass-inset-surface px-2 py-1.5 text-sm text-foreground text-center"
                          />
                          <button
                            type="button"
                            onClick={() => {
                  ...
                  {workload.cluster && workload.kind ? (
                    <Link
                      ...
                      className="px-3 py-1.5 text-sm font-medium rounded glass-inset-surface text-muted hover:bg-white/[0.08] border glass-divider transition-colors"
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-subtle">Runtime</span><p className="text-foreground">{workload.runtime}</p></div>
              <div><span className="text-subtle">Image</span><p className="text-foreground font-mono text-xs">{workload.image}</p></div>
              <div><span className="text-subtle">Status</span><p className="text-foreground">{workload.status}</p></div>
              <div><span className="text-subtle">Created</span><p className="text-foreground">{workload.created_at ? formatTimestamp(workload.created_at) : ''}</p></div>
              {workload.cluster && <div><span className="text-subtle">Cluster</span><p className="text-foreground">{workload.cluster}</p></div>}
              {workload.namespace && <div><span className="text-subtle">Namespace</span><p className="text-foreground">{workload.namespace}</p></div>}
              {workload.kind && <div><span className="text-subtle">Kind</span><p className="text-foreground">{workload.kind}</p></div>}
              <div><span className="text-subtle">Source</span><p className="text-foreground capitalize">{workload.source ?? 'aether'}</p></div>
              {!isAetherManaged && clusterHealth ? (
                <div className="col-span-2" data-testid="workload-health-summary">
                  <span className="text-subtle">Cluster health</span>
            ...
            {!isAetherManaged && clusterDetail && clusterDetail.pods.length > 0 ? (
              <div className="mt-4" data-testid="workload-pod-info">
                <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Pods</h4>
                <div className="overflow-x-auto rounded-lg border glass-divider">
                  <table className="w-full min-w-[28rem] text-left text-xs">
                    <thead>
                      <tr className="glass-inset-surface text-subtle">
                    ...
                    <tbody>
                      {clusterDetail.pods.map((pod) => (
                        <tr key={pod.name} className="glass-divider-t">
                          ...
                          <td className="px-3 py-2 text-muted">{pod.node ?? '—'}</td>
                          <td className="px-3 py-2 text-right whitespace-nowrap">
                            <button
                              ...
                              className="rounded px-1.5 py-0.5 text-[11px] text-subtle transition-colors hover:bg-white/5 hover:text-foreground"
                              title="Copy pod name"
                            >
                              Copy
                            </button>
            ...
            {!isAetherManaged && clusterDetail && clusterDetail.conditions.length > 0 ? (
              <div className="mt-4" data-testid="workload-conditions">
                <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Conditions</h4>
            ...
            {!isAetherManaged && clusterDetail && ((clusterDetail.owner_references?.length ?? 0) > 0 || (clusterDetail.owned_resources?.length ?? 0) > 0) ? (
              <div className="mt-4 grid gap-4 sm:grid-cols-2" data-testid="workload-ownership">
                {(clusterDetail.owner_references?.length ?? 0) > 0 ? (
                  <div>
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Owned by</h4>
                    <ul className="space-y-1 text-xs text-muted">
                      {clusterDetail.owner_references!.map((owner) => (
                        <li key={`${owner.kind}/${owner.name}`} className="font-mono">
                          {owner.kind}/{owner.name}
                          {owner.controller ? <span className="ml-1 text-subtle">(controller)</span> : null}
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {(clusterDetail.owned_resources?.length ?? 0) > 0 ? (
                  <div>
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Owns</h4>
                    <ul className="max-h-28 space-y-1 overflow-auto text-xs text-muted">
              ...
              <details className="mt-4 rounded-lg border glass-divider p-3" data-testid="workload-manifest-insights">
                  ...
                  <div data-testid="workload-image-drift">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(observedImages.length > 0 ? observedImages : containerResources.map((c) => c.image).filter(Boolean) as string[]).map((image) => (
                        <code key={image} className="rounded glass-inset-surface px-2 py-1 text-[11px] text-muted">{image}</code>
                      ))}
                    </div>
                  </div>
                ) : null}
                {containerResources.length > 0 ? (
                  <div data-testid="workload-resources">
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Resources</h4>
                    <div className="overflow-x-auto rounded-lg border glass-divider">
                        ...
                        <tbody>
                          {containerResources.map((container) => (
                            <tr key={container.name} className="glass-divider-t">
                              <td className="px-3 py-2 font-mono text-foreground">{container.name}</td>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {labelEntries.length > 0 ? (
                      <div data-testid="workload-labels">
                        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Labels</h4>
                        <div className="max-h-28 space-y-1 overflow-auto text-[11px]">
                          {labelEntries.map(([key, value]) => (
                            <div key={key} className="flex gap-2 font-mono">
                              ...
                              <span className="truncate text-muted" title={value}>{value}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {annotationEntries.length > 0 ? (
                      <div data-testid="workload-annotations">
                        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Annotations</h4>
                        <div className="max-h-28 space-y-1 overflow-auto text-[11px]">
            ...
            {!isAetherManaged && (clusterMetrics.length > 0 || clusterEvents.length > 0) ? (
              <div className="mt-4" data-testid="workload-live-ops">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-semibold uppercase tracking-wider text-subtle">Live ops</span>
                  <button
                    type="button"
                ...
                {clusterMetrics.length > 0 ? (
                  <div data-testid="workload-live-metrics">
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Live usage</h4>
                    <div className="overflow-x-auto rounded-lg border glass-divider">
                        ...
                        <tbody>
                          {clusterMetrics.map((metric) => (
                            <tr key={metric.name} className="glass-divider-t">
                              <td className="px-3 py-2 font-mono text-foreground truncate max-w-[12rem]" title={metric.name}>{metric.name}</td>
                              <td className="px-3 py-2 text-muted">{metric.cpu}</td>
                              <td className="px-3 py-2 text-muted">{metric.memory}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {clusterEvents.length > 0 ? (
                  <div data-testid="workload-recent-events">
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Recent cluster events</h4>
                      ...
                      {clusterEvents.map((event, index) => (
                        <div key={`${event.timestamp}-${event.reason}-${index}`} className="rounded glass-inset-surface px-2.5 py-1.5">
                          <div className="flex items-center gap-2 text-[11px]">
                            ...
                            <span className="ml-auto text-subtle">{event.timestamp ? formatTimestamp(event.timestamp) : ''}</span>
                          </div>
                          <p className="mt-0.5 truncate text-[11px] text-muted" title={event.message}>{event.message}</p>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {!isAetherManaged && workload.kind === 'HelmRelease' && helmHistory.length > 0 ? (
              <div className="mt-4 rounded-lg border glass-divider p-3" data-testid="workload-helm-history">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  <h4 className="text-xs font-semibold uppercase tracking-wider text-subtle">Helm revisions</h4>
                  {canMutate ? (
                    <div className="flex items-center gap-2">
                      <select
                        value={helmRevision}
            ...
            {!isAetherManaged && isRolloutClusterKind(workload.kind) ? (
              <div className="mt-4 rounded-lg border glass-divider p-3" data-testid="workload-rollout">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  <h4 className="text-xs font-semibold uppercase tracking-wider text-subtle">Rollout</h4>
                  {rollout ? (
                    <span className="text-xs text-muted">{rollout.status || 'unknown'}</span>
                  ) : (
                ...
                {rollout && rollout.history.length > 0 ? (
                  <div className="mt-3 max-h-28 overflow-auto text-xs">
                    {rollout.history.slice(0, 5).map((rev) => (
                      <div key={rev.revision} className="flex gap-2 glass-divider-t py-1 text-muted">
                        <span className="font-mono text-muted">#{rev.revision}</span>
            ...
            {!isAetherManaged && canMutate && canPortForwardClusterKind(workload.kind) && (workload.kind === 'Service' || (clusterDetail?.pods.length ?? 0) > 0 || portForwardPod) ? (
              <div className="mt-4 rounded-lg border glass-divider p-3" data-testid="workload-port-forward">
                <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-subtle">Port forward</h4>
            ...
            {!isAetherManaged && workload.cluster ? (() => {
              ...
              return (
                <details className="mt-4" data-testid="workload-kubectl">
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-xs font-semibold uppercase tracking-wider text-subtle">
                  ...
                  <div className="mt-2 space-y-1.5">
                    {cmds.map((cmd) => (
                      ...
                      >
                        <span className="w-24 shrink-0 text-[11px] font-medium uppercase tracking-wider text-subtle">
                          {cmd.label}
                        </span>
                        <code className="min-w-0 flex-1 truncate font-mono text-xs text-muted" title={cmd.command}>
                          {cmd.command}
                        </code>
                        <button
                          type="button"
            ...
            {isAetherManaged ? (
              <div className="mt-4 glass-drawer p-3" data-testid="workload-snapshots">
                <h4 className="mb-2 text-sm font-semibold text-foreground">Snapshots</h4>
                {snapshotsLoading ? (
                  <p className="text-xs text-subtle">Loading snapshots…</p>
                ) : snapshots.length === 0 ? (
                  <p className="text-xs text-subtle">No snapshots yet — snapshots are created before migrations and updates.</p>
                ) : (
                  <ul className="space-y-2">
                    {snapshots.map((snap) => (
                      <li key={snap.version} className="flex flex-wrap items-center justify-between gap-2 text-sm">
                        <span className="font-mono text-xs text-muted truncate" title={snap.path}>
                          v{snap.version}
                        </span>
                        {canMutate ? (
                          <button
            ...
            {isKubeWorkload && (
              <div className="mt-4 glass-drawer p-3">
                <h4 className="mb-3 text-sm font-semibold text-foreground">Network policies</h4>
                <dl className="space-y-2 text-sm">
                  <div className="flex justify-between gap-4">
                    <dt className="text-subtle">NetworkPolicy</dt>
                    <dd className="font-mono text-xs text-foreground">{networkPolicyName}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-subtle">Cilium CNP</dt>
                    <dd className="font-mono text-xs text-foreground">{ciliumPolicyName}</dd>
                  </div>
                </dl>
                <a
        ...
        {activeTab === 'manifest' && (
          <div className="space-y-4">
            {isAetherManaged ? (
              <p className="text-subtle">Manifest inspection is currently available for Kubernetes resources discovered directly from the cluster.</p>
            ) : !clusterDetail ? (
              <p className="text-subtle">Loading Kubernetes resource details...</p>
            ) : (
              <>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-subtle">API Version</span><p className="text-foreground">{clusterDetail.api_version ?? 'unknown'}</p></div>
                  <div><span className="text-subtle">Pods</span><p className="text-foreground">{clusterDetail.pods.length}</p></div>
                </div>

                {clusterDetail.conditions.length > 0 && (
                  <div className="glass-drawer p-3">
                    <h4 className="mb-3 text-sm font-semibold text-foreground">Conditions</h4>
                ...
                {clusterDetail.pods.length > 0 && (
                  <div className="glass-drawer p-3">
                    <h4 className="mb-3 text-sm font-semibold text-foreground">Pods</h4>
                    <div className="space-y-2">
                      {clusterDetail.pods.map((pod) => (
                        <div key={pod.name} className="grid grid-cols-5 gap-3 glass-table-row rounded-md px-3 py-2 text-sm">
                          <div className="col-span-2">
                            <div className="text-foreground">{pod.name}</div>
                            <div className="text-xs text-subtle">{pod.node ?? 'node unknown'}</div>
                          </div>
                          <div className="text-muted">{pod.phase}</div>
                          <div className="text-muted">{pod.ready}/{pod.total_containers} ready</div>
                          <div className="text-muted">{pod.restarts} restarts</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="glass-drawer p-3">
                  <h4 className="mb-3 text-sm font-semibold text-foreground">Manifest</h4>
              ...
              <div>
                ...
                {((driftData as Record<string, unknown>).drifts as Array<Record<string, string>>)?.map((d, i: number) => (
                  <div key={i} className="glass-inset-surface rounded p-2 mb-2 text-sm">
                    ...
                    <div className="text-subtle mt-1">Expected: {d.expected} | Actual: {d.actual}</div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-subtle">Loading drift data...</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'events' && (
          <div className="text-sm">
            ...
            {eventsLoading ? (
              <p className="text-subtle">Loading events...</p>
            ) : events.length === 0 ? (
              <p className="text-subtle">No events recorded for this workload.</p>
            ) : (
                ...
                {events.map((ev, i) => (
                  <div key={`${ev.timestamp}-${i}`} className="flex items-start gap-3 glass-drawer p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-subtle mt-1">{ev.timestamp ? formatTimestamp(ev.timestamp) : ''}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center justify-between glass-divider-b glass-inset-surface px-5 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-xs px-3 py-1 rounded glass-inset-surface glass-inset-hover text-muted disabled:opacity-50"
                ...
                <button
                  ...
                  className="text-xs px-3 py-1 rounded glass-inset-surface glass-inset-hover text-muted"
                >
                  Copy
                </button>
                <button
                ...
                <button
                  ...
                  className="text-muted hover:text-foreground text-xl leading-none ml-1"
                >
                  ×
                </button>
              </div>
