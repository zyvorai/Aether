...
export default function AiProvidersPage() {
  const [registry, setRegistry] = useState<ZeusProviderRegistry | null>(null);
  ...
  const load = useCallback(async () => {
    const data = await apiFetch<ZeusProviderRegistry>('/zeus/providers');
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-2">
          {(registry?.providers ?? []).map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-xl border border-white/10 p-3">
              ...
              <div className="flex gap-2">
                <button type="button" className="btn-secondary text-xs" onClick={() => void testProvider(p.id)}>
                  <Zap className="h-3 w-3" />
                </button>
                <button type="button" className="btn-secondary text-xs text-red-300" onClick={() => void removeProvider(p.id)}>
          ...
          {(registry?.providers.length ?? 0) === 0 ? (
            <p className="text-sm text-slate-500">No providers configured — env vars used as bootstrap defaults.</p>
          ) : null}
        </div>
      </div>
    </section>
