...
export default function ConfidentialWorkloadPanel({ workloadName, runtime }: ConfidentialWorkloadPanelProps) {
  ...
  if (loading) {
    return <p className="text-sm text-slate-500 py-4">Loading trust and attestation data…</p>;
  }
    ...
    return (
      <div className="py-4 space-y-2">
        <p className="text-sm text-slate-400">
          ...
          <code className="text-slate-300">confidential:</code> block with{' '}
          <code className="text-slate-300">enabled: true</code> in the workload spec (KubeVirt runtime).
        </p>
    ...
    <GlassSection variant="panel" accent="red" className="space-y-6 py-2" testId="confidential-workload-panel">
      ...
      <div className="glass-divider-b pb-4">
        ...
        {sovereignVerdict ? (
          ...
          <p className="text-xs text-slate-500">Evaluate region lock, BYOK, and offline attestation policy for this spec.</p>
        )}
      </div>
      ...
      {migrationPlan && (
        <div className="glass-divider-b pb-4">
          ...
          />
          <p className="text-xs font-mono text-slate-500 break-all mt-2">
      ...
      {network && (
        <div className="glass-divider-b pb-4">
          ...
          {network.spiffe_id && (
            <p className="text-xs font-mono text-slate-500 break-all mb-2">{network.spiffe_id}</p>
          )}
      ...
      {meta?.image_digest && (
        <div>
          <h4 className="text-sm font-medium text-slate-300 mb-2">Measured launch digest</h4>
          <code className="text-xs text-slate-400 break-all block mb-2">{meta.image_digest}</code>
          <Badge
      ...
      {trust && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-medium text-slate-300">Composite trust score</h4>
            <Badge
          ...
          {trust.spiffe_id && (
            <p className="mt-3 text-xs font-mono text-slate-500 break-all">SPIFFE: {trust.spiffe_id}</p>
          )}
        </div>
      )}
      ...
      {status ? (
        <div className="glass-divider-t pt-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <span className="text-slate-500 block text-xs mb-1">Measurement drift</span>
              <Badge text={status.drift ? 'drift detected' : 'stable'} variant={status.drift ? 'red' : 'green'} />
            </div>
            {status.baseline_digest && (
              <div className="col-span-2">
                <span className="text-slate-500 block text-xs mb-1">Baseline digest</span>
                <code className="text-xs text-slate-400 break-all">{status.baseline_digest}</code>
              </div>
            )}
            <div className="col-span-2 text-xs text-slate-600">Updated {status.updated_at}</div>
          </div>
          <button
            type="button"
      ...
      <div className="glass-divider-t pt-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-500">Launch digest (optional)</label>
            <input
              type="text"
        ...
        <div>
          <label className="mb-1 block text-xs text-slate-500">Guest report (base64)</label>
          <textarea
      ...
      <div className="glass-divider-t pt-4">
        ...
        {guestkitMessage && (
          <p className="text-xs text-slate-400 mb-2">{guestkitMessage}</p>
        )}
          ...
          <div className="space-y-2 max-h-40 overflow-auto" data-testid="guestkit-history-panel">
            {guestkitHistory.slice().reverse().map((entry) => (
              <div key={entry.inspected_at} className="glass-table-row rounded text-xs p-2">
                ...
                <p className="text-slate-500">{entry.findings.join(' · ')}</p>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-xs text-slate-600" data-testid="guestkit-history-empty">
      ...
      {!notConfidential && (
        <div className="glass-divider-t pt-4" data-testid="attest-secrets-panel">
          ...
          {secretStatus.length > 0 ? (
            ...
            <p className="text-xs text-slate-500" data-testid="attest-secrets-empty">
