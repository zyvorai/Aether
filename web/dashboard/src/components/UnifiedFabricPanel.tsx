      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-semibold text-ink">Unified Edge + Cloud Fabric</h2>
            <p className="text-sm text-ink-2">Edge agents linked to federated clusters and workloads</p>
          </div>
        </div>
        <button type="button" onClick={() => void load()} className="rounded-xl border glass-divider px-3 py-2 text-xs text-ink-2 transition hover:border-brand/30">
      ...
      <ul className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {(report?.nodes ?? []).slice(0, 12).map((n) => (
          <li key={n.id} className="rounded-xl border glass-divider glass px-3 py-2 text-sm backdrop-blur-sm">
            ...
            <p className="mt-1 text-xs text-ink-3">{n.detail}</p>
          </li>
        ))}
      </ul>
    </section>
