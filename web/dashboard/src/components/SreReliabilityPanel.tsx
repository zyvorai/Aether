    ...
    >{schedule ? (
          ...
          {(timeline?.entries ?? []).slice(0, 12).map((e) => (
            <li key={e.id} className="rounded-lg border glass-divider px-3 py-2 text-sm">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-subtle">{e.timestamp}</span>
          ...
          {(budgets?.entries ?? []).slice(0, 8).map((e) => (
            <li key={e.workload} className="rounded-lg border glass-divider px-3 py-2 text-sm text-muted">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="font-medium text-foreground">{e.workload}</span>
      ...
      {tab === 'postmortem' ? (
        <div data-testid="postmortem-panel">
          <p className="mb-3 text-sm font-medium text-foreground">{postmortem?.title}</p>
          {(postmortem?.sections ?? []).map((s) => (
            <div key={s.heading} className="mb-4">
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-subtle">{s.heading}</h3>
          ...
          <ul className="space-y-2">
            {(onCall?.channels ?? []).map((c) => (
              <li key={c.provider} className="flex items-center gap-2 rounded-lg border glass-divider px-3 py-2 text-sm">
                ...
                <span className="text-xs text-subtle">{c.env_hint}</span>
              </li>
            ))}
          </ul>
        </div>
          ...
          {(chaos?.experiments ?? []).slice(0, 6).map((e) => (
            <li key={e.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border glass-divider px-3 py-2 text-sm">
              <div>
                ...
                <p className="text-xs text-subtle">{e.description}</p>
              </div>
              <button
                type="button"
        ...
        <ul className="space-y-3" data-testid="game-days-panel">
          {(gameDays?.scenarios ?? []).map((s) => (
            <li key={s.id} className="rounded-lg border glass-divider px-3 py-2 text-sm">
              ...
              <ul className="ml-6 list-disc text-xs text-muted">
                {s.steps.map((step) => (
                  <li key={step}>{step}</li>
                ))}
              </ul>
      ...
      {!loading && tab === 'postmortem' && postmortem ? (
        <details className="mt-4">
          <summary className="cursor-pointer text-sm text-primary">View markdown</summary>
          <pre className="mt-2 max-h-48 overflow-auto rounded-xl border glass-divider glass-code-block-body p-3 text-xs text-muted">
            {postmortem.markdown}
          </pre>
        </details>
      ) : null}
