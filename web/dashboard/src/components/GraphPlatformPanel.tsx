    ...
    >{k8sImport ? (
      ...
      {tab === 'impact' ? (
        <div data-testid="graph-impact-panel">
          <p className="mb-3 text-sm text-slate-300">{impact?.summary}</p>
          <ul className="space-y-1">
            {(impact?.downstream ?? []).map((w) => (
              <li key={w} className="text-sm text-slate-400">
      ...
      {tab === 'blast' ? (
        <div data-testid="graph-blast-radius-panel">
          ...
          <p className="mb-2 text-sm text-slate-400">{blast?.summary}</p>
          <ul className="space-y-1">
        ...
        <ul className="space-y-2" data-testid="graph-placement-panel">
          {(placement?.entries ?? []).slice(0, 8).map((e) => (
            <li key={e.workload} className="rounded-lg border glass-divider px-3 py-2 text-sm text-slate-300">
              ...
              {e.co_locate_with ? (
                <span className="ml-2 text-xs text-slate-500">co-locate with {e.co_locate_with}</span>
              ) : null}
            </li>
          ))}
        </ul>
