    ...
    >
      ...
      {!loading && tab === 'terraform' ? (
        <div data-testid="labs-terraform-panel" className="space-y-3">
          ...
          {terraform ? (
            <pre className="glass overflow-x-auto p-3 text-xs text-muted font-mono max-h-48">
              {terraform.hcl.slice(0, 400)}…
            </pre>
          ) : (
            <p className="text-sm text-subtle">Click to generate OpenTofu-ready HCL from intent pipeline.</p>
          )}
        </div>
      ) : null}
        ...
        <div data-testid="labs-community-panel" className="space-y-3">
          ...
          <ul className="space-y-1 text-sm text-muted">
            {community.community.slice(0, 5).map((c) => (
              <li key={c.id}>
                {c.title} <span className="text-subtle">★ {c.stars}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {!loading && tab === 'carbon' && carbon ? (
        <div data-testid="labs-carbon-panel">
          ...
          {carbon.workloads.length > 0 ? (
            <ul className="mt-2 text-xs text-subtle space-y-1">
            ...
            <p className="text-xs text-subtle mt-2">Deploy workloads to see per-line carbon estimates.</p>
          )}
        </div>
      ) : null}
