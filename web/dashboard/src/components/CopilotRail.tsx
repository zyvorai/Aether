    ...
    >
      <div className="relative z-[1] flex items-center gap-3 glass-table-row px-4 py-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold text-white">Ask Aether</div>
            ...
            {agents.map((agent) => {
              ...
              return (
                <button
                  ...
                  className={`copilot-agent-chip ${active ? 'copilot-agent-chip-active' : ''}`}
                  data-testid={`copilot-agent-${agent.id}`}
                >
              ...
              <div className="flex flex-col gap-1.5">
                {visiblePrompts.map((prompt) => (
                  <button
                    ...
                    data-testid={`copilot-prompt-${prompt.slice(0, 20).replace(/\s+/g, '-').toLowerCase()}`}
                  >
                    {prompt}
                  </button>
                ))}
