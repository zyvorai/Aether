...
Use `make build` for a debug Rust build and `make release` for an optimized binary. Run `make test` for the full Rust test suite and `make lint` for `cargo fmt --check` plus strict `clippy`. `make ci` runs the expected pre-PR checks. For quick spec validation, use `make validate` or `cargo run --release -- validate --spec workload.yaml`. For the dashboard, run `cd web/dashboard && npm run dev` for local development and `npm run build` for a production bundle.
