---
hero:
  eyebrow: LEGAL
  title: Order Form (template)
  tone: pink
---

**DRAFT — NOT FOR EXECUTION WITHOUT LEGAL REVIEW**

| Field | Value |
|-------|--------|
| Order Form # | OF-________ |
| MSA date | __________ |
| Customer legal name | __________ |
| Billing address | __________ |
| Term start | __________ |
| Term end | __________ (12 months unless stated) |
| Auto-renew | ☐ Yes ☐ No |

---

## Products and tier

| Product | Edition | Deployment |
|---------|---------|------------|
| PacketWolf / NetPredator | ☐ Enterprise | ☐ Self-hosted ☐ Hosted |
| Ragnarok | ☐ Enterprise | ☐ Self-hosted |
| Aether | ☐ Enterprise | ☐ Self-hosted |
| GuestKit | ☐ Enterprise | ☐ Self-hosted |

**Tier:** ☐ Professional  ☐ Enterprise  ☐ Sovereign  ☐ Hyperscale (custom)

---

## Licensed metrics (transparent)

| Metric | Quantity | Overage |
|--------|----------|---------|
| Production clusters | | |
| CPU sockets (optional) | | |
| Confidential / TEE nodes | | |
| Tenant trust domains | | |
| GPU confidential pools | | |
| Monitored workloads (PacketWolf) | | |
| Named support contacts | | |

---

## Fees

| Line item | Annual (INR/USD) |
|-----------|------------------|
| Subscription | |
| Support (SLA tier) | |
| Professional services (SOW ref) | |
| **Total** | |

Payment: ☐ Annual upfront ☐ Quarterly Net 30

---

## Documents incorporated

☐ MSA  ☐ ELA  ☐ SLA  ☐ DPA  ☐ AUP  ☐ Export Compliance

**Special terms:** __________________________________________

---

**ZyvorAI Labs Private Limited**  
By: __________________________  Name: __________________________  Title: **Director**

**Customer**  
By: __________________________  Name: __________________________  Title: __________________________
