...
| Kind | Support |
|------|---------|
| Deployment / Job | ✅ single volume `{name}-pvc`. |
| StatefulSet | ✅ one Atlas volume per replica, named `{name}-storage-{name}-{ordinal}`, which the StatefulSet **adopts** instead of dynamically provisioning. |
| KubeVirt / Metal3 / Podman | Not routed through Atlas (native/no persistent storage). |

**StatefulSet scale-up caveat:** volumes are provisioned for the *declared* replica
count at deploy time. Scaling a StatefulSet beyond that count later requires
provisioning the additional Atlas volumes first (the `atlas/…` class in the
`volumeClaimTemplate` is dormant while all PVCs are pre-created and would not resolve
for a new ordinal).
